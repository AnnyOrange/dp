...
# 定义参数
CHECKPOINTS=('data/checkpoints/tool_ph_2.ckpt')  # 填入多个 checkpoint
DEVICES=(3)                                          # 对应每个任务使用的 GPU 设备，和上面的checkpoint对应
SPEEDS=(1 2 3 4)                                         # 要使用的 speed 参数，也就是几倍速
OUTPUTDIRS=('data/tool_eval/')        # 对应每个 checkpoint 的 output_dir
